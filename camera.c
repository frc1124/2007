...
extern rom const char sqrt[];

// camera T packet structure
T_Packet_Data_Type T_Packet_Data;
Cam_Target_Data_Structure Targets;

void update_targets(void) {
	int gobble;
	if (T_Packet_Data.pixels < 1) {
		Targets.num_of_lights = 0;
	}else if (T_Packet_Data.confidence > dual_target_threshold) {
		Targets.num_of_lights = 1;
		Targets.c_light_angle = ACENT(T_Packet_Data.my);
	}else{
		gobble = (T_Packet_Data.y2 - T_Packet_Data.y1)/2;
		Targets.num_of_lights = 2;
		Targets.r_light_angle = ACENT(T_Packet_Data.y1 + sqrt[T_Packet_Data.pixels]);
		Targets.c_light_angle = ACENT(gobble);
		Targets.l_light_angle = ACENT(T_Packet_Data.y2 - sqrt[T_Packet_Data.pixels]);
	}
}

char cam_hasTarget(void) {
	char oops = IMAGE_WIDTH - T_Packet_Data.mx;
	return (oops > -2 && oops < 2);
}
